...
public class ClienteService {
  ...
  public List<Cliente> searchAdvanced(AdvancedSearchRequest req) {
	  ...
	  try {
	    if (req.createdFrom() != null && !req.createdFrom().isBlank()) from = LocalDate.parse(req.createdFrom());
	    if (req.createdTo()   != null && !req.createdTo().isBlank())   to   = LocalDate.parse(req.createdTo());
	  } catch (Exception e) {
  ...
  public String exportCsv() {
	  ...
	  repo.findAll().forEach(c -> sb
	      .append(escapeCsv(c.sharedKey())).append(',')
	      .append(escapeCsv(c.businessId())).append(',')
